...
interface DynamicEditAccountContentProps {
  accountId: string
}

export function DynamicEditAccountContent({ accountId }: DynamicEditAccountContentProps) {
  ...
  const loadFieldConfigurations = async (companyId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Initialize form data with account values if available
        if (accountData) {
  ...
  useEffect(() => {
    if (accountData && fieldConfigs.length > 0) {
      const initialData: Record<string, any> = {}
      fieldConfigs
        .filter(f => f.is_enabled && f.field_name !== 'account_id')
        .forEach(field => {
        ...
        <div className="bg-white border-b border-gray-200 px-6 py-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-1">Please verify all changes before updating the account</p>
            </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-gray-600">Update the account details below</p>
            </div>
          </div>
          <Badge variant="outline" className="text-sm">
            Step 1 of 2
