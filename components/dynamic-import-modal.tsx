...
interface DynamicImportModalProps {
  ...
  moduleType: 'accounts' | 'contacts' | 'products' | 'leads'
  isImporting?: boolean
...
export function DynamicImportModal({
  ...
  const loadFieldConfigurations = async () => {
    ...
    try {
      ...
      const apiEndpoint = `/api/admin/${moduleType === 'accounts' ? 'account' : moduleType === 'contacts' ? 'contact' : moduleType === 'products' ? 'product' : 'lead'}-fields?companyId=${companyId}`
